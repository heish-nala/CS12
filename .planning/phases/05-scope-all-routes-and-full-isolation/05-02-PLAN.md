...
Note: `GET/POST /api/overview-widgets` is intentionally EXCLUDED from this migration. That route imports all its data functions from `@/lib/mock-data` (`getOverviewWidgetsByClient`, `createOverviewWidget`, `calculateAggregation`, `calculateChartData`) — it returns mock/in-memory data only. While it does call `checkDsoAccess` for auth gating, migrating it would be wasted effort since the route will be rewritten when real widget persistence is added. It poses no cross-org data leak risk because the underlying data is not real.
...
10. **`GET /api/tasks`** — Has TWO auth paths that both need migration:
    - **When `dso_id` IS provided** (line ~19): Replace `checkDsoAccess(userId, dsoId)` with `requireOrgDsoAccess(request, dsoId)`. Note: `dsoId` is `string | null` from `searchParams.get()`, but this code is inside an `if (dsoId)` guard so TypeScript narrows it to `string` — no type issue.
    - **When `dso_id` is NOT provided** (Category B enumeration, line ~49): The bare `user_dso_access` query (`select('dso_id').eq('user_id', userId)`) has NO org filter — a user could see tasks from DSOs in other orgs. Fix: After resolving `userId`, call `getUserOrg(userId)`. If no org, return the empty `{ tasks: [], task_groups: [], total: 0 }` response. Replace the bare `user_dso_access` query with the org-filtered join pattern:
      ```typescript
      ...
      ```
    - Import both `requireOrgDsoAccess` and `getUserOrg` from `@/lib/auth`.
