...
export function createDataTable(clientId: string, name: string, templateId?: string): DataTable {
    ...
    if (template) {
        ...
        // Populate sample data for doctors template
        if (templateId === 'doctors') {
            const sampleDoctors = [
                { name: 'Dr. Sarah Johnson', email: 'sarah.johnson@clinic.com', phone: '555-0101', startDate: '2024-02-01', status: 'active', casesSubmitted: 45, coursesCompleted: 8, lastContact: '2024-11-25', notes: 'Top performer' },
                { name: 'Dr. Michael Chen', email: 'michael.chen@clinic.com', phone: '555-0102', startDate: '2024-03-15', status: 'active', casesSubmitted: 32, coursesCompleted: 6, lastContact: '2024-11-20', notes: 'Needs follow-up on certifications' },
                { name: 'Dr. Emily Rodriguez', email: 'emily.rodriguez@clinic.com', phone: '555-0103', startDate: '2024-01-10', status: 'completed', casesSubmitted: 78, coursesCompleted: 12, lastContact: '2024-11-28', notes: 'Completed all requirements' },
                { name: 'Dr. James Wilson', email: 'james.wilson@clinic.com', phone: '555-0104', startDate: '2024-04-01', status: 'at_risk', casesSubmitted: 12, coursesCompleted: 2, lastContact: '2024-10-15', notes: 'Behind on timeline' },
                { name: 'Dr. Lisa Martinez', email: 'lisa.martinez@clinic.com', phone: '555-0105', startDate: '2024-05-01', status: 'active', casesSubmitted: 28, coursesCompleted: 5, lastContact: '2024-11-22', notes: 'Making good progress' },
                { name: 'Dr. Robert Taylor', email: 'robert.taylor@clinic.com', phone: '555-0106', startDate: '2024-02-15', status: 'active', casesSubmitted: 56, coursesCompleted: 9, lastContact: '2024-11-26', notes: 'Excellent engagement' },
                { name: 'Dr. Jennifer Lee', email: 'jennifer.lee@clinic.com', phone: '555-0107', startDate: '2024-06-01', status: 'inactive', casesSubmitted: 5, coursesCompleted: 1, lastContact: '2024-09-10', notes: 'On leave' },
                { name: 'Dr. David Anderson', email: 'david.anderson@clinic.com', phone: '555-0108', startDate: '2024-03-01', status: 'active', casesSubmitted: 41, coursesCompleted: 7, lastContact: '2024-11-24', notes: 'Consistent performer' },
            ];
            ...
            sampleDoctors.forEach(doctor => {
                const row: DataRow = {
                    ...
                    table_id: table.id,
                    data: {
                        [columnIds['Name']]: doctor.name,
                        [columnIds['Email']]: doctor.email,
                        [columnIds['Phone']]: doctor.phone,
                        [columnIds['Start Date']]: doctor.startDate,
                        [columnIds['Status']]: doctor.status,
                        [columnIds['Cases Submitted']]: doctor.casesSubmitted,
                        [columnIds['Courses Completed']]: doctor.coursesCompleted,
                        [columnIds['Last Contact']]: doctor.lastContact,
                        [columnIds['Notes']]: doctor.notes,
                    },
                    created_at: new Date().toISOString(),
                    updated_at: new Date().toISOString(),
                };
...
// ============================================================================
// INITIALIZE ATTENDEE TRACKER WITH DOCTOR DATA
// ============================================================================

let initialized = false;

// Helper to populate empty doctors tables with sample data
function populateEmptyDoctorsTables(): void {
    const doctorsTables = mockDataTables.filter(t => t.template_id === 'doctors');

    for (const table of doctorsTables) {
        const rows = mockDataRows.filter(r => r.table_id === table.id);
        if (rows.length > 0) continue; // Already has data

        const columns = mockDataColumns.filter(c => c.table_id === table.id);
        const columnIds: Record<string, string> = {};
        columns.forEach(col => {
            columnIds[col.name] = col.id;
        });

        // Only populate if we have the required columns
        if (!columnIds['Name']) continue;

        const sampleDoctors = [
            { name: 'Dr. Sarah Johnson', email: 'sarah.johnson@clinic.com', phone: '555-0101', startDate: '2024-02-01', status: 'active', casesSubmitted: 45, coursesCompleted: 8, lastContact: '2024-11-25', notes: 'Top performer' },
            { name: 'Dr. Michael Chen', email: 'michael.chen@clinic.com', phone: '555-0102', startDate: '2024-03-15', status: 'active', casesSubmitted: 32, coursesCompleted: 6, lastContact: '2024-11-20', notes: 'Needs follow-up' },
            { name: 'Dr. Emily Rodriguez', email: 'emily.rodriguez@clinic.com', phone: '555-0103', startDate: '2024-01-10', status: 'completed', casesSubmitted: 78, coursesCompleted: 12, lastContact: '2024-11-28', notes: 'Completed' },
            { name: 'Dr. James Wilson', email: 'james.wilson@clinic.com', phone: '555-0104', startDate: '2024-04-01', status: 'at_risk', casesSubmitted: 12, coursesCompleted: 2, lastContact: '2024-10-15', notes: 'Behind' },
            { name: 'Dr. Lisa Martinez', email: 'lisa.martinez@clinic.com', phone: '555-0105', startDate: '2024-05-01', status: 'active', casesSubmitted: 28, coursesCompleted: 5, lastContact: '2024-11-22', notes: 'Good progress' },
            { name: 'Dr. Robert Taylor', email: 'robert.taylor@clinic.com', phone: '555-0106', startDate: '2024-02-15', status: 'active', casesSubmitted: 56, coursesCompleted: 9, lastContact: '2024-11-26', notes: 'Excellent' },
            { name: 'Dr. Jennifer Lee', email: 'jennifer.lee@clinic.com', phone: '555-0107', startDate: '2024-06-01', status: 'inactive', casesSubmitted: 5, coursesCompleted: 1, lastContact: '2024-09-10', notes: 'On leave' },
            { name: 'Dr. David Anderson', email: 'david.anderson@clinic.com', phone: '555-0108', startDate: '2024-03-01', status: 'active', casesSubmitted: 41, coursesCompleted: 7, lastContact: '2024-11-24', notes: 'Consistent' },
        ];
        ...
        sampleDoctors.forEach(doctor => {
            const row: DataRow = {
                ...
                data: {
                    [columnIds['Name']]: doctor.name,
                    ...(columnIds['Email'] && { [columnIds['Email']]: doctor.email }),
                    ...(columnIds['Phone'] && { [columnIds['Phone']]: doctor.phone }),
                    ...(columnIds['Start Date'] && { [columnIds['Start Date']]: doctor.startDate }),
                    ...(columnIds['Status'] && { [columnIds['Status']]: doctor.status }),
                    ...(columnIds['Cases Submitted'] && { [columnIds['Cases Submitted']]: doctor.casesSubmitted }),
                    ...(columnIds['Courses Completed'] && { [columnIds['Courses Completed']]: doctor.coursesCompleted }),
                    ...(columnIds['Last Contact'] && { [columnIds['Last Contact']]: doctor.lastContact }),
                    ...(columnIds['Notes'] && { [columnIds['Notes']]: doctor.notes }),
                },
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString(),
            };
...
export function initializeAttendeeTracker(clientId: string): void {
    if (initialized) return;
    initialized = true;

    // Also populate any empty doctors tables
    populateEmptyDoctorsTables();

    // Time tracking metric IDs
    const scansMetricId = `metric-${nextPeriodId++}`;
    const acceptedMetricId = `metric-${nextPeriodId++}`;
    const diagnosedMetricId = `metric-${nextPeriodId++}`;

    // Create Attendee Tracker table with time tracking enabled
    const table: DataTable = {
        id: String(nextTableId++),
        client_id: clientId,
        name: 'Attendee Tracker',
        description: 'Track doctors through the blueprint program',
        icon: 'users',
        color: 'blue',
        is_template: false,
        template_id: null,
        ...
        time_tracking: {
            ...
            frequency: 'monthly',
            metrics: [
                { id: scansMetricId, name: 'Scans', type: 'number' },
                { id: acceptedMetricId, name: 'Accepted', type: 'number' },
                { id: diagnosedMetricId, name: 'Diagnosed', type: 'number' },
            ],
        },
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
    ...
    // Create columns - main table columns (excluding time-tracked metrics which are in period data)
    const columns: Omit<DataColumn, 'id' | 'created_at' | 'updated_at'>[] = [
        { table_id: table.id, name: 'Doctor', type: 'text', config: {}, is_required: true, is_primary: true, width: 180, order_index: 0 },
        { table_id: table.id, name: 'Blueprint', type: 'percentage', config: {}, is_required: false, is_primary: false, width: 100, order_index: 1 },
        { table_id: table.id, name: 'Phone', type: 'phone', config: {}, is_required: false, is_primary: false, width: 140, order_index: 2 },
        { table_id: table.id, name: 'Email', type: 'email', config: {}, is_required: false, is_primary: false, width: 220, order_index: 3 },
        { table_id: table.id, name: 'Status', type: 'status', config: { options: [
            { id: 'a1', value: 'active', label: 'Active', color: 'green' },
            { id: 'a2', value: 'at_risk', label: 'At Risk', color: 'orange' },
            { id: 'a3', value: 'inactive', label: 'Inactive', color: 'gray' },
            { id: 'a4', value: 'completed', label: 'Completed', color: 'blue' }
        ]}, is_required: false, is_primary: false, width: 120, order_index: 4 },
    ];
    ...
    // Attendee data with monthly breakdown
    const attendees = [
        { name: 'Alexandria Wenig', blueprint: 0, phone: '14196012934', email: 'drwenig@puresmiles.com', status: 'at_risk', monthlyData: generateMonthlyData(0, 0, 0) },
        { name: 'Celeana Lambing', blueprint: 92, phone: '15133353247', email: 'drlambing@puresmiles.com', status: 'active', monthlyData: generateMonthlyData(80, 3, 0) },
        { name: 'Chris Holland', blueprint: 8, phone: '16145630367', email: 'drholland@puresmiles.com', status: 'at_risk', monthlyData: generateMonthlyData(4, 0, 3) },
        { name: 'Cindy Liu', blueprint: 67, phone: '17163031803', email: 'drliu@puresmiles.com', status: 'active', monthlyData: generateMonthlyData(29, 0, 1) },
        { name: 'Dr. Sarah Johnson', blueprint: 85, phone: '555-0101', email: 'sarah.johnson@smiledental.com', status: 'active', monthlyData: generateMonthlyData(162, 11, 17) },
        { name: 'Dr. Michael Chen', blueprint: 73, phone: '555-0102', email: 'michael.chen@smiledental.com', status: 'active', monthlyData: generateMonthlyData(104, 8, 12) },
        { name: 'Dr. Emily Rodriguez', blueprint: 91, phone: '555-0103', email: 'emily.rodriguez@pacific.com', status: 'active', monthlyData: generateMonthlyData(192, 15, 22) },
        { name: 'Dr. James Wilson', blueprint: 15, phone: '555-0104', email: 'james.wilson@pacific.com', status: 'at_risk', monthlyData: generateMonthlyData(48, 2, 3) },
        { name: 'Dr. Lisa Martinez', blueprint: 78, phone: '555-0105', email: 'lisa.martinez@aspen.com', status: 'active', monthlyData: generateMonthlyData(104, 9, 14) },
        { name: 'Dr. Robert Taylor', blueprint: 62, phone: '555-0106', email: 'robert.taylor@smiledental.com', status: 'active', monthlyData: generateMonthlyData(87, 6, 9) },
        { name: 'Dr. Jennifer Lee', blueprint: 0, phone: '555-0107', email: 'jennifer.lee@pacific.com', status: 'inactive', monthlyData: generateMonthlyData(12, 1, 0) },
        { name: 'Dr. David Anderson', blueprint: 54, phone: '555-0108', email: 'david.anderson@aspen.com', status: 'active', monthlyData: generateMonthlyData(76, 5, 8) },
        { name: 'Dr. Amanda Foster', blueprint: 88, phone: '555-0109', email: 'amanda.foster@smiledental.com', status: 'active', monthlyData: generateMonthlyData(145, 12, 18) },
        { name: 'Dr. Kevin Park', blueprint: 42, phone: '555-0110', email: 'kevin.park@pacific.com', status: 'at_risk', monthlyData: generateMonthlyData(58, 3, 5) },
        { name: 'Dr. Rachel Green', blueprint: 96, phone: '555-0111', email: 'rachel.green@aspen.com', status: 'completed', monthlyData: generateMonthlyData(210, 18, 25) },
        { name: 'Dr. Thomas Wright', blueprint: 71, phone: '555-0112', email: 'thomas.wright@smiledental.com', status: 'active', monthlyData: generateMonthlyData(98, 7, 11) },
    ];
    ...
    attendees.forEach(attendee => {
        const row: DataRow = {
            ...
            table_id: table.id,
            data: {
                [columnIds['Doctor']]: attendee.name,
                [columnIds['Blueprint']]: attendee.blueprint,
                [columnIds['Phone']]: attendee.phone,
                [columnIds['Email']]: attendee.email,
                [columnIds['Status']]: attendee.status,
            },
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
        };
        ...
        // Create period data for each month
        for (let month = 0; month < 12; month++) {
            const startDate = new Date(currentYear, month, 1);
            ...
            const period: PeriodData = {
                ...
                metrics: {
                    [scansMetricId]: attendee.monthlyData[month].scans,
                    [acceptedMetricId]: attendee.monthlyData[month].accepted,
                    [diagnosedMetricId]: attendee.monthlyData[month].diagnosed,
                },
                created_at: new Date().toISOString(),
                updated_at: new Date().toISOString(),
            };
...
// Helper to generate realistic monthly data that sums to totals
function generateMonthlyData(totalScans: number, totalAccepted: number, totalDiagnosed: number) {
    const months: { scans: number; accepted: number; diagnosed: number }[] = [];
    const currentMonth = new Date().getMonth();

    let remainingScans = totalScans;
    let remainingAccepted = totalAccepted;
    let remainingDiagnosed = totalDiagnosed;

    for (let i = 0; i < 12; i++) {
        if (i > currentMonth) {
            // Future months have 0
            months.push({ scans: 0, accepted: 0, diagnosed: 0 });
        } else if (i === currentMonth) {
            // Current month gets remaining
            months.push({
                scans: remainingScans,
                accepted: remainingAccepted,
                diagnosed: remainingDiagnosed,
            });
        } else {
            // Past months get a random portion
            const factor = Math.random() * 0.3 + 0.05; // 5-35% per month
            const scans = Math.min(Math.floor(totalScans * factor), remainingScans);
            const accepted = Math.min(Math.floor(totalAccepted * factor), remainingAccepted);
            const diagnosed = Math.min(Math.floor(totalDiagnosed * factor), remainingDiagnosed);

            months.push({ scans, accepted, diagnosed });
            remainingScans -= scans;
            remainingAccepted -= accepted;
            remainingDiagnosed -= diagnosed;
        }
    }
